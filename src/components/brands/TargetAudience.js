  ...
  return (
    <section className="bg-white py-16 px-4 md:px-8">
      <div className="max-w-4xl mx-auto flex flex-col items-center text-center">
        {data.title && (
          <h2 className="text-3xl font-extrabold text-[#9402c0] mb-6">
            {data.title}
          </h2>
        )}
        ...
        {data.suitableFor && data.suitableFor.length > 0 && (
          <div className="flex flex-wrap justify-center gap-4 mb-12 w-full max-w-3xl">
            {data.suitableFor.map((item, idx) => (
              <div 
                key={idx}
                className={`bg-white px-6 py-4 rounded-lg shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] border border-gray-100 text-gray-700 text-sm md:text-base flex-grow text-center transition-all duration-200 hover:border-violet-400 hover:text-violet-600 ${
