  ...
  return (
    <section className="bg-[#dceaff] py-16 px-4 md:px-8">
      <div className="max-w-5xl mx-auto flex flex-col items-center text-center">
        {data.title && (
          <h2 className="text-3xl font-extrabold text-[#9402c0] mb-4">
            {data.title}
          </h2>
        )}
        ...
        {data.cards && data.cards.length > 0 && (
           <div className="flex flex-col md:flex-row items-stretch justify-center gap-4 w-full my-8">
            {data.cards.map((card, idx) => (
              <div 
                key={idx} 
                className="bg-white p-6 md:p-8 rounded-xl flex-1 flex items-center justify-center text-center shadow-sm"
              >
